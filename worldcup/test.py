...
def load_ecup():
    ...
    for match_file in os.listdir(os.path.join(os.path.dirname(__file__), 'test_data', 'E_cup', 'match_data')):
        ...
        db.match.update(
            {"id": match_obj.id},
            {"$set": {"handicap": (match_data['HandicapA'], match_data['HandicapB'])}, "$set": {"id" : match_date.strftime("%Y%m%d") + match_data['teamA'] + match_data['teamB']}}
...
def test_Match_update_profit_and_loss_no_auction(choice_a, choice_b, no_choice, handicap, score_a, score_b, expected, auction1):
    # prepare gamblers
